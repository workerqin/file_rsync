...
def dorsync(src_str, rsyncExePath, srcPath, dstPath, rsyncPwFile):
    ...
    #result = os.system("rsync.exe -vzrtopgu  --delete --progress  /cygdrive/D/test root@119.29.120.47::qin --password-file=/cygdrive/E/cwrsync/rsync_client/rsyncd.password")
    cmd = "%s -vzrtpu --delete --progress %s %s --password-file=%s" % (rsyncExePath, srcPath, dstPath, rsyncPwFile)
...
def update(rsyncExePath, updateSrcPath, updateDstPath, rsyncPwFile):
    #os.chdir("E:/cwrsync/rsync_client/")
    print "update"
    cmd = "%s -vzrtpu --progress %s %s --password-file=%s"%(rsyncExePath, updateSrcPath, updateDstPath, rsyncPwFile)
    #print cmd
    result = os.system(cmd)
